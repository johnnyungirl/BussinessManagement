...
func (u userRepository) UpdateUser(user model.User) (model.User, error) {
	...
	if err := u.DB.First(&new_user, "id=?", user.ID).Error; err != nil {
		return new_user, err
	}
	return new_user, u.DB.Model(&new_user).Updates(&user).Error
}

func (u userRepository) DeleteUser(user model.User) (model.User, error) {
	if err := u.DB.First(&user, "id=?", user.ID).Error; err != nil {
		return user, err
	}
